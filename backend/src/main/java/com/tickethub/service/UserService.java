...
public class UserService {
    ...
    public List<TechnicianAvailabilityResponse> getTechniciansAvailability() {
        ...
        return technicians.stream().map(tech -> {
            long count = ticketRepository.
                countByAssignedTechnicianIdAndStatusIn(tech.getId(),
                    List.of(TicketStatus.ACCEPTED, TicketStatus.IN_PROGRESS));

            String fullName = java.util.stream.Stream.of(tech.getPrenom(), tech.
                getNom())
     ...
     */
    @Transactional
    public void createUserByAdmin(final RegisterRequest request) {
    ...
    /**
     * Retrieves a summary of all users.
     *
    ...
    /**
     * Retrieves a summary of all pending users.
     *
     * @return a list of pending user summaries
     */
    ...
    private UserSummaryDTO toUserSummaryDTO(final User user) {
        ...
        String fullName = java.util.stream.Stream.of(user.getPrenom(), user.
            getNom())
                .filter(value -> value != null && !value.isBlank())
                .collect(Collectors.joining(" "));
        if (fullName.isBlank()) {
