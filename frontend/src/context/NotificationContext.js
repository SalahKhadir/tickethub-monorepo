...
export function NotificationProvider({ children }) {
  ...
  const openSSE = useCallback(() => {
    ...
    const token =
      typeof window !== "undefined" ? localStorage.getItem("th_token") : null;
        ...
        setNotifications((prev) => {
          ...
          setTimeout(() => {
            if (!seenRef.current.has(String(newTicket.id))) {
              ...
              toast.success(
                `New Ticket: ${newTicket.title || `TH-${newTicket.id}`}`,
                {
                  duration: 4000,
                  style: {
                    background: "#111C2D",
    ...
    es.onerror = () => {
      ...
      if (retryCountRef.current > MAX_RETRIES) {
        console.warn(
          `SSE: max retries (${MAX_RETRIES}) reached. Giving up until next login.`,
        );
        return;
      }
      const delay =
        retryCountRef.current === 1
          ? 3_000
          : Math.min(5_000 * retryCountRef.current, 30_000);
      console.warn(
        `SSE connection error — retrying in ${delay / 1000}s… (attempt ${retryCountRef.current}/${MAX_RETRIES})`,
      );
