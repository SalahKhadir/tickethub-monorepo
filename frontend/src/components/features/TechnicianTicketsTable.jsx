...
export default function TechnicianTicketsTable({
  tickets,
  page = 0,
  totalPages = 0,
  onPageChange,
  ...
  if (!tickets || tickets.length === 0) {
    return (
      <div className="py-6 flex flex-col items-center justify-center">
        <span className="text-gray-400 text-sm">No tickets assigned yet.</span>
      </div>
    );
  }
          ...
          <tbody>
            {tickets.map((ticket) => {
              ...
              return (
                <tr
                  key={ticket.id}
                  className={`border-t border-gray-100 hover:bg-white transition border-l-4 ${
                    ticket.priority === "CRITICAL"
