...
export default function TicketActions({
  ...
  const isAssignedToMe = (() => {
    ...
    const userNames = [user?.fullName, user?.username, user?.email, user?.name]
      .filter(Boolean)
      .map(lower);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-gray-100 bg-blue-50/30 p-4 shadow-sm">
      ...
      {error ? (
        <p
          className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600 font-medium"
          role="alert"
        >
          {error}
        </p>
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={
            loading || (actionConfig.requiresSolution && !solution.trim())
          }
        >
