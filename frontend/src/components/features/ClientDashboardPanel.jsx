    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(
        (t) =>
          String(t?.id || "")
            .toLowerCase()
            .includes(term) || (t?.title || "").toLowerCase().includes(term),
      );
    }
      ...
      {!loading && filteredTickets.length > 0 && (
        <div className="space-y-3">
          {filteredTickets.map((ticket) => {
            ...
            const isAuthor =
              userIdentifiers.includes(ticket?.createdBy) ||
              userIdentifiers.includes(ticket?.clientName);

            return (
              <div key={ticket.id}>
                <div
                  className={`rounded-2xl border bg-white px-6 py-4 shadow-sm ${getCardBorderClass(ticket.priority)}`}
                >
                ...
                {deletingTicketId === ticket.id && (
                  <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 flex items-center justify-between">
                    <p className="text-red-600 text-sm font-medium">
                      Are you sure you want to delete this ticket? This cannot
                      be undone.
                    </p>
