...
export default function NotificationDropdown({ onClose }) {
  ...
  const ticketRoute = (id) => {
    ...
    if (role === ROLES.TECHNICIAN)
      return `${ROUTES.TECHNICIAN_TICKETS}?highlight=${id}`;
      ...
      <ul className="max-h-80 overflow-y-auto divide-y divide-[#1F2937]">
        {notifications.length === 0 ? (
          ...
          notifications.map((ticket) => {
            ...
            const roleMap =
              STATUS_TEXT_BY_ROLE[role] || STATUS_TEXT_BY_ROLE[ROLES.CLIENT];
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`h-2 w-2 rounded-full shrink-0 ${DOT_STYLES[status] || "bg-gray-400"}`}
                      />
