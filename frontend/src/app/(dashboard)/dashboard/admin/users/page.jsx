...
const parseError = (err) => {
  if (err?.response?.status === 403)
    return "Accès refusé : Droits insuffisants (ROLE_ADMIN requis).";
...
const roleBadge = (roles) => {
  ...
  return (
    <span
      className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${map[r] || "bg-gray-100 text-gray-500 border border-gray-200"}`}
    >
      {r || "UNKNOWN"}
    </span>
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Approbation des inscriptions et gestion des comptes
          </p>
        </div>
        {tab === 1 && (
          <button
