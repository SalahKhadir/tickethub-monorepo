...
const calcResolutionTime = (ticket) => {
  if (!ticket.createdAt || !ticket.updatedAt)
    return { display: "N/A", breached: false };
  const diff =
...
export default function TechnicianHistoryPage() {
  ...
  const categories = useMemo(
    () => Object.keys(groupedTickets).sort(),
    [groupedTickets],
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            All resolved and closed tickets assigned to you
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {categories.map((cat) => {
            ...
            const validTimes = catTickets.filter(
              (t) => t.createdAt && t.updatedAt,
            );
            const totalMs = validTimes.reduce(
              (sum, t) =>
                sum +
            ...
            const avgH = validTimes.length
              ? Math.floor(totalMs / validTimes.length / 3_600_000)
              : 0;
            const avgM = validTimes.length
              ? Math.floor(((totalMs / validTimes.length) % 3_600_000) / 60_000)
              : 0;
                            ...
                            return (
                              <Fragment key={ticket.id}>
                                ...
                                >
                                  ...
                                  <td
                                    className="px-6 py-4 font-medium text-gray-900 max-w-[200px] truncate"
                                    title={ticket.title}
                                  >
                                    {ticket.title}
                                  </td>
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap">
                                    <span className="flex items-center gap-1.5">
                                      ...
                                      />
                                      <span
                                        className={`font-medium ${res.breached ? "text-red-600" : "text-gray-700"}`}
                                      >
                                        {res.display}
                                      </span>
                                      {res.breached && (
