...
const TicketCard = ({ ticket, onStartWork, onResolveClick }) => {
  ...
  const showSlaTimer =
    isCritical && status !== "RESOLVED" && status !== "CLOSED";
  ...
  const authorSubtext =
    authorName && ticket?.authorEmail ? ticket.authorEmail : "";
  ...
  return (
    <div
      className={`bg-white rounded-2xl border p-4 shadow-sm mb-3 flex flex-col h-full ${borderClass} ${isCritical ? "critical-card bg-[#FFF5F5]" : ""}`}
    >
      ...
      {/* Title */}
      <h3 className="text-gray-900 font-medium text-sm mt-2">
      ...
      {showSlaTimer && slaTime && (
        ...
        >
          ...
          <span>
            ...
            {slaTime.overdue ? "overdue" : "remaining"}
          </span>
        </div>
      )}
...
export default function TechnicianTicketsPage() {
  ...
  const isTechnician =
    String(user?.role || "").toLowerCase() === ROLES.TECHNICIAN;
  ...
  const {
    ...
    loading,
    refresh,
  } = useTickets(["NEW", "ACCEPTED", "IN_PROGRESS"]);
    ...
    async (ticketId) => {
      ...
      try {
        ...
        const message =
          ...
          err?.message ||
          "Failed to resolve ticket. Try again.";
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  TH-{resolveTicketId} · Provide a resolution summary
                </p>
              </div>
              <button
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    setResolveSolution(e.target.value);
                    setResolveError("");
                  }}
                  placeholder="Describe what was done to resolve this issue..."
                  rows={5}
                  disabled={resolving}
                  className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 transition resize-none disabled:opacity-60 ${
                    resolveError
                ...
                <p className="mt-1.5 text-xs text-gray-400">
                  {resolveSolution.trim().length} characters · minimum 10
                  recommended
                </p>
              </div>
            </div>
